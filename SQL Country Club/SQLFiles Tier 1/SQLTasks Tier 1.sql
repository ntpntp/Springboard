...
SELECT f.facid, f.name, SUM( b.slots) AS hours_of_usage
        FROM Bookings AS b
...
SELECT strftime('%m', b.starttime ) AS
        MONTH , f.facid, f.name, SUM(b.slots) AS hours_of_usage
        FROM Bookings AS b
        LEFT JOIN Facilities AS f ON b.facid = f.facid
        WHERE b.memid !=0
